	...
	class SDS_Theme_Options {
		...
		function sds_theme_options_sanitize( $input ) {
			...
			if ( isset( $input['create_child_theme'] ) ) {
				...
				if ( empty( $child_theme_name ) ) {
					add_settings_error( 'sds_theme_options', 'child-theme-name-invalid', __( 'Please enter a valid child theme name.', 'modern-estate' ) );

					return $input;
				}
				// Make sure the requested child theme does not already exist
				else if ( file_exists( $theme_root . $child_theme_name ) ) {
					add_settings_error( 'sds_theme_options', 'child-theme-exists', __( 'It appears that a child theme with that name already exists. Please try again with a different child theme name.', 'modern-estate' ) );
				...
				// Make sure child theme creation didn't fail
					...
					add_settings_error( 'sds_theme_options', 'child-theme-creation-failed', __( 'There was a problem creating the child theme. Please check your server permissions.', 'modern-estate' ) );
				...
				// Child theme was created successfully
					...
					if ( isset( $input['child_theme']['activate'] ) ) {
						...
						add_settings_error( 'sds_theme_options', 'child-theme-activation-success', sprintf( __( 'New theme activated. <a href="%s">Visit site</a> or <a href="%s">edit child theme</a>.', 'modern-estate' ), home_url( '/' ), admin_url( 'theme-editor.php?theme=' . urlencode( $child_theme_name ) ) ), 'updated' );
					}
					else
						add_settings_error( 'sds_theme_options', 'child-theme-creation-success', sprintf( __( 'Child theme created successfully. <a href="%s">Activate</a> your child theme.', 'modern-estate' ), wp_nonce_url( admin_url( 'themes.php?action=activate&amp;stylesheet=' . urlencode( $child_theme_name ) ), 'switch-theme_' . $child_theme_name ) ), 'updated' );
				}
			}
		...
		?>
			<div class="wrap about-wrap">
				...
				<div class="about-text sds-about-text"><?php printf( _x( '%1$s', 'Theme options panel descripton', 'modern-estate' ), self::$options_page_description ); ?></div>
				...
				<h2 class="nav-tab-wrapper sds-theme-options-tab-wrap">
					...
					<a href="#one-click-child-themes" id="one-click-child-themes-tab" class="nav-tab sds-theme-options-tab"><?php _e( 'One-Click Child Themes', 'modern-estate' ); ?></a>
				...
				<form method="post" action="options.php" enctype="multipart/form-data" id="sds-theme-options-form">
					...
					<div id="one-click-child-themes-tab-content" class="sds-theme-options-tab-content">
						<h3><?php _e( '"One-Click" Child Themes', 'modern-estate' ); ?></h3>
						...
						<?php if ( ! $this->get_child_theme_activation_status() && is_child_theme() ) : // Child theme is currently active ?>
							<div class="message error sds-child-themes-message" style="border-left: 4px solid #ffba00; display: none !important;">
								<p><strong><?php _e( 'Please Note: It looks like you\'re already using a child theme.', 'modern-estate' ); ?></strong></p>
							</div>
						<?php endif; ?>

						<div class="form-table">
							<p><?php printf( __( 'Child themes are an essential part to <a href="%1$s" target="_blank">WordPress theme modification</a>. If you\'re looking to enhance <strong>%2$s</strong> beyond the Theme Options we\'ve provided, you\'ll find the tools to easily create your very own child theme below. It couldn\'t be more simpler.', 'modern-estate' ), 'http://slocumthemes.com/2013/12/how-to-create-a-child-theme/', $this->theme->get( 'Name' ) ); ?></p>

							<?php if ( ! empty( $this->child_themes ) ) : // Child themes exist ?>
								<h4><?php printf( __( 'The following %1$s child themes already exist', 'modern-estate' ), $this->theme->get( 'Name' ) ); ?></h4>
								<ul class="sds-child-themes">
									...
									?>
												<li><?php printf( _x( '<strong>%1$s (Active)</strong> - <a href="%2$s">Edit</a>', 'Child theme name and edit link', 'modern-estate' ), $child_theme->get( 'Name' ), admin_url( 'theme-editor.php?theme=' . urlencode( $child_theme->get_stylesheet() ) ) ); ?></li>
									<?php
											else:
									?>
												<li><?php printf( _x( '%1$s - <a href="%2$s">Activate</a>', 'Child theme name and activation link', 'modern-estate' ), $child_theme->get( 'Name' ), wp_nonce_url( admin_url( 'themes.php?action=activate&amp;stylesheet=' . urlencode( $child_theme->get_stylesheet() ) ), 'switch-theme_' . $child_theme->get_stylesheet() ) ); ?></li>
									<?php
											endif;
										endforeach;
									?>
						...
						<h3><?php _e( '1. Name Your Child Theme', 'modern-estate' ); ?> <span class="description"><?php _ex( '(required)', 'This field is required', 'modern-estate' ); ?></span></h3>

						<table class="form-table">
							<tr valign="top">
								<th scope="row"><?php _e( 'Child Theme Name:', 'modern-estate' ); ?></th>
								<td>
						...
						<h3><?php _e( '2. Advanced Settings', 'modern-estate' ); ?> <span class="description"><?php _ex( '(optional)', 'This field is optional', 'modern-estate' ); ?></span></h3>

						<table class="form-table">
							<tr valign="top">
								<th scope="row"><?php _e( 'Activate Child Theme:', 'modern-estate' ); ?></th>
								<td>
									<div class="checkbox sds-themes-child-themes-checkbox">
										...
										<label for="sds_theme_options_one_click_child_theme_activate"><?php _e( 'Activate child theme once it has been created', 'modern-estate' ); ?></label>
									</div>
									<span class="description"><?php printf( __( 'This option will also keep navigation menus and widgets from %1$s.', 'modern-estate' ), $this->theme->get( 'Name' ) ); ?></span>
								</td>
							</tr>
						</table>

						<p class="one-click-child-theme-submit">
							<?php submit_button( __( 'Create Child Theme', 'modern-estate' ), 'primary', 'sds_theme_options[create_child_theme]', false ); ?>
						</p>
					</div>
		...
		function create_stylesheet_header( $theme_name ) {
			...
			$stylesheet_header['description'] = sprintf( __( 'A "one-click" child theme created for %1$s - %2$s', 'modern-estate' ), $this->theme->get( 'Name' ), $stylesheet_header['description'] );
